# Requirements Archive: v0.3 TypeScript 6 Migration & Toolchain Modernization
...
# Requirements: DxKit — v0.3 TypeScript 6 Migration & Toolchain Modernization
...
- [x] **ROB-06**: `loadManifests()`'s `dapps` and inline `manifests` tiers validate array shape via a shared `coerceManifestArray()` helper; a wrong-shape value emits `dx:error` (source `shell:manifest`) instead of throwing before `window.__DXKIT__` is exposed (closes v0.3 milestone-audit CR-01 — the untrusted-input parity gap ROB-05 left in the two non-registry tiers)
...
- **TS7-01**: Migrate core + plugins to TypeScript 7.1 once a stable point release lands (the payoff of v0.3's forward-compat groundwork)
...
| Feature | Reason |
|---------|--------|
| TypeScript 7.x migration | The whole point of v0.3 is to *prepare* for it; TS7 is fresh and we're waiting on a stable 7.1 point release for ABI/API |
| tsup → tsdown migration | tsdown needs Node ≥22.18 and is a build-tool swap; v0.3 stays on maintained `tsup@8.5.x`. Deferred to next milestone |
...
*Last updated: 2026-07-19 — added ROB-06 (Phase 10) closing v0.3 milestone-audit CR-01, extending ROB-05's array-shape guard to the `dapps` and inline `manifests` tiers*
