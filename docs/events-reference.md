...
| `shell:manifest` | A **present** `dapps`/`manifests` config value (or a `registryUrl`-fetched `200` body) is **not an array** — every manifest tier is shape-checked by the shared `coerceManifestArray()` guard. A wrong-shape value emits one error and fail-closes that tier to an empty manifest list instead of throwing an uncaught `TypeError` before `window.__DXKIT__` is exposed. A **nullish** `dapps`/`manifests` (`null`/`undefined`) is treated as unset and falls through silently — only a present non-array value errors. The `registryUrl` `200`-body check fires **ungated** (even on the default `/registry.json` probe, unlike the non-OK/parse cases above); the `dapps`/`manifests` checks guard developer-supplied config for untyped IIFE/static-HTML consumers |
