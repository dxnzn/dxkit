...
| `shell:manifest` | A `dapps`, inline `manifests`, or `registryUrl`-fetched value is **not an array** — every manifest tier is shape-checked by the shared `coerceManifestArray()` guard. A wrong-shape value emits one error and fail-closes that tier to an empty manifest list instead of throwing an uncaught `TypeError` before `window.__DXKIT__` is exposed. The `registryUrl` `200`-body check fires **ungated** (even on the default `/registry.json` probe, unlike the non-OK/parse cases above); the `dapps`/`manifests` checks guard developer-supplied config for untyped IIFE/static-HTML consumers |
